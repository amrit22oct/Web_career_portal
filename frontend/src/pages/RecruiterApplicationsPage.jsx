...
const RecruiterApplicationsPage = () => {
  ...
  if (!user)
    return (
      <div className="text-center mt-10 text-lg text-gray-700">Loading user info…</div>
    );
  if (loading)
    return (
      <div className="text-center mt-10 text-lg text-gray-700">Loading applications…</div>
    );
  if (error)
    return (
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-gradient-to-r from-indigo-50 via-purple-50 to-pink-50 rounded-lg shadow-xl">
      <h1 className="text-4xl font-extrabold mb-8 text-center text-indigo-900 tracking-wide drop-shadow-md">
      ...
      <div className="mb-6 flex justify-end">
        <select
          ...
          className="border border-indigo-400 bg-white text-indigo-900 font-semibold rounded-lg px-4 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 hover:ring-indigo-400 transition w-40 sm:w-48"
      ...
      {paginatedApps.length === 0 ? (
        ...
        paginatedApps.map((app) => (
          <div
            key={app._id}
            className="bg-white rounded-2xl p-6 mb-6 shadow-lg hover:shadow-2xl transition-shadow duration-300 border border-indigo-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6"
          >
            <div className="sm:flex-1 min-w-0">
              <h2 className="text-2xl font-bold text-indigo-800 truncate">{app.student?.name || "N/A"}</h2>
              <p className="text-indigo-600 font-medium truncate">Email: {app.student?.email || "N/A"}</p>
              <p className="text-indigo-600 font-medium truncate">University: {app.student?.university || "N/A"}</p>
              <p className="text-indigo-600 font-medium truncate">
                Skills: {app.student?.skills?.join(", ") || "N/A"}
              </p>
              <p className="text-indigo-700 font-semibold mt-2 truncate">Job: {app.job?.title || "N/A"}</p>

              <p className="mt-3 text-indigo-600 truncate">
                Resume:{" "}
                {app.resumeLink ? (
                  <>
                    <a
              ...
              {app.coverLetter && (
                <p className="mt-3 italic text-indigo-700 border-l-4 border-indigo-400 pl-4 bg-indigo-50 rounded-md shadow-inner truncate">
            ...
            <div className="flex gap-4 sm:flex-col sm:items-end">
              <button
                className={`px-5 py-2 rounded-full font-semibold shadow-md transition transform hover:scale-105 hover:shadow-xl text-white w-28 sm:w-full ${
                ...
                aria-label={`Accept application from ${app.student?.name || "student"}`}
              >
                Accept
              </button>
              <button
                ...
                aria-label={`Reject application from ${app.student?.name || "student"}`}
              >
                Reject
              </button>
            </div>
        ...
        <div className="flex flex-wrap justify-center mt-8 gap-3">
          {Array.from({ length: totalPages }, (_, i) => (
            <button
              ...
              className={`px-4 py-2 rounded-full font-semibold transition min-w-[40px] ${
                currentPage === i + 1
