...
const JobsPage = () => {
  ...
  return (
    <div className="bg-[#0f172a] min-h-screen py-8 px-4 sm:px-6 md:px-10 w-full">
    <div className="max-w-7xl mx-auto">
      <h1 className="text-white text-3xl sm:text-4xl font-bold mb-8 text-center">
        Explore Career Opportunities
      </h1>
  
      {user?.role === 'recruiter' && (
        <>
          <h2 className="text-white text-2xl font-semibold mb-4 text-center sm:text-left">Your Posted Jobs</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {visibleMyJobs.length > 0 ? (
              visibleMyJobs.map((job) => (
                <div key={job._id || job.id} className="bg-gray-800 p-5 rounded-lg shadow-md">
                  <JobCard job={job} />
                </div>
              ))
            ) : (
              <div className="col-span-full text-gray-400 italic text-center mt-4">
      ...
      <h2 className="text-white text-2xl font-semibold mb-4 text-center sm:text-left">All Public Job Listings</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {jobs.length > 0 ? (
          jobs.map((job) => (
            <div key={job._id || job.id} className="bg-gray-800 p-5 rounded-lg shadow-md">
